  ...
  for (var i = 0; i < list.length; i++) {
    if (list[i].name.includes("shard")) {
      ...
      if ("decodedBodySize" in list[i])
        console.log(
          "... decodedBodySize[" + i + "] = " + list[i].decodedBodySize
        );
      else console.log("... decodedBodySize[" + i + "] = NOT supported");

      if ("encodedBodySize" in list[i])
        console.log(
          "... encodedBodySize[" + i + "] = " + list[i].encodedBodySize
        );
      else console.log("... encodedBodySize[" + i + "] = NOT supported");

      if ("transferSize" in list[i])
        console.log("... transferSize[" + i + "] = " + list[i].transferSize);
      else console.log("... transferSize[" + i + "] = NOT supported");
    }
  }
}
...
async function run() {
  try {
    ...
    if (err.message == "Failed to fetch") {
      status(
        "Error! Failed to fetch the neural network models. Try disabling your ad blocker as this may prevent models from being downloaded from a different domain. (There are no ads here)"
      );
    }
  }
}
...
async function load_model() {
  ...
  console.log(
    "First Model loaded " + Math.floor(performance.now() - startTime) + "ms"
  );
  ...
  if (typeof SYSTEM.AEMODEL_PATH !== "undefined") {
    ...
    console.log(
      "Second Model loaded " + Math.floor(performance.now() - startTime) + "ms"
    );
  }
...
async function run_demo() {
  ...
  imgElement.onload = () => {
    predict(
      ...
      true,
      "Example Image (" +
        imgElement.src.substring(imgElement.src.lastIndexOf("/") + 1) +
        ")"
    );
  };
...
function prepare_image(thispred, imgElement) {
  ...
  thispred[0].img_highres = prepare_image_resize_crop(
    imgElement,
    Math.max(imgElement.width, imgElement.height)
  );

  thispred[0].img_resized = prepare_image_resize_crop(
    imgElement,
    MODEL_CONFIG.IMAGE_SIZE
  );

  thispred[0].img_input = thispred[0].img_resized
    .mul(2)
    .sub(1)
...
async function predict_real(imgElement, isInitialRun, name) {
  ...
  thispred.find(".loading").each((k, v) => {
    v.style.display = "block";
  });
  thispred[0].id = name;
  thispred[0].grads = []; // to cache grads
  SYSTEM.predictionsElement.insertBefore(
    thispred[0],
    SYSTEM.predictionsElement.firstChild
  );
  ...
  thispred.find(".inputimage_highres").di;
  ...
  console.log(
    "Prepared input image " + Math.floor(performance.now() - startTime) + "ms"
  );
  ...
  if (typeof SYSTEM.AEMODEL_PATH !== "undefined") {
    ...
    if (isInitialRun && (recScore > 0.27 || recScore < 0.01)) {
      error = new Error(
        "Something wrong with this browser. Try refreshing the page. (" +
          recScore +
          ")"
      );
    ...
    console.log(
      "Wrote images " + Math.floor(performance.now() - startTime) + "ms"
    );
    ...
    console.log(
      "Computed SSIM " + Math.floor(performance.now() - startTime) + "ms"
    );
    ...
    can_predict = ssim.ssim > 0.6;

    //	////////////////////

    console.log(
      "Plotted Reconstruction " +
        Math.floor(performance.now() - startTime) +
        "ms"
    );
  } else {
  ...
  if (!can_predict) {
    ...
    output = tf.tidy(() => {
      const batched = thispred[0].img_input.reshape([
        1,
        1,
        MODEL_CONFIG.IMAGE_SIZE,
        MODEL_CONFIG.IMAGE_SIZE,
...
async function computeGrads_real(thispred, idx) {
  ...
  if (thispred[0].grads[idx] == undefined) {
    ...
    layer = tf.tidy(() => {
      chestgrad = tf.grad((x) =>
        SYSTEM.chesternet.predict(x).reshape([-1]).gather(idx)
      );
  ...
  thispred
    .find(".desc")
    .text("Predictive regions for " + MODEL_CONFIG.LABELS[idx]);
}
...
function showProbErrorColor(predictionContainer) {
  ...
  row.style.width = "100%";
  row.textContent =
    "This image appears to be a color image and we suspect it is not an xray.";
  predictionContainer.appendChild(row);
}
...
function showProbResults(thispred) {
  ...
  for (let i = -1; i < classes.length; i++) {
    ...
    if (i == -1 || classes[i].probability <= 0.6) {
      ...
      $(explainElement).click(function () {
        computeGrads(thispred, i, explainElement);
      });
    }
...
async function showResults(imgElement, layers, classes, recScore) {
  ...
  if (recScore > 0.35) {
    ...
    row.className = "row";
    row.textContent =
      "This image is too far out of our training distribution so we will not process it. (recScore:" +
      Math.round(recScore * 100) / 100 +
      ")";
    probsContainer.appendChild(row);
  } else {
    const row = document.createElement("div");
    ...
    for (let i = 0; i < classes.length; i++) {
      ...
      scale = parseInt((1 - classes[i].probability) * 255);
      probsElement.style.backgroundColor =
        "rgb(255," + scale + "," + scale + ")";
      row.appendChild(probsElement);
      probsContainer.appendChild(row);
    }
